...
<div class="page-header">
  <h1 class="page-title">Edit Devices Process</h1>
  <ol class="breadcrumb">
      ...
      <div class="panel">
        <div class="panel-body container-fluid">
          ...
          <div class="example-wrap">
            <h4 class="example-title">Form Edit Process for Device <?= (empty($data->name))?'':$data->name;  ?> </h4>
            <form method="post" autocomplete="off">
                <div class="row">
                        ...
                        <div class="form-group form-material">
                                ...
                                <div class="input-group">
                                    <div class="form-control-wrap">
                                        <input type="number" class="form-control" id="inputNumber" name="number" autocomplete="off" value="<?= count($item->var); ?>" required/>
                                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group form-material">
                                ...
                                <textarea class="form-control empty" rows="3" id="inputPre" name="pre"><?= (empty($item->pre))?'':$item->pre;  ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group form-material">
                            <label class="form-control-label" for="inputProcess">Code for Process</label>
                            <input type="text" class="form-control"  name="process" placeholder="" id="inputProcess" autocomplete="off" value="<?= (empty($item->process))?'':$item->process;  ?>"/>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group form-material">
                  <button type="submit" name="save" value="save" class="btn btn-primary">Update Process</button>&nbsp; &nbsp;
                  <a href="<?= base_url();?>schema"><button type="button" class="btn btn-default">Cancel</button></a>
                  <input type="hidden"  id="inputCode" value="<?= $id ?>"/>
                  <input type="hidden" id="inputoldfield"  name="oldfield" value="<?= $field ?>"/>                  
                </div>
            </form>
          </div>
...
<script type="text/javascript">
  var item = "";
  var varlist = ['<?= implode($item->var,"', '") ?>'];
  function deviceForm(){
    var device = $("#inputCode").val();
    var field = $("#inputoldfield").val();
