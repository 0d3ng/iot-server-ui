...
class Interfaces extends CI_Controller {
	...
	public function add(){   
        ...
        if($this->input->post('save')){  
            ...
            $respo = $this->interface_m->add($input);
            if($respo->status){             
		...
		$this->load->view('interface_add_v', $data);
	}
	...
	public function edit($id){       
		...
		$data['success']='';
		$data['error']='';
		$data['title']= 'Data interface Edit';		
		$data['user_now'] = $this->session->userdata('dasboard_iot');	
        $query = array(
            "add_by" => $data['user_now']->id
        );
        ...
        if($this->input->post('save')){  
            $idinterface = $this->input->post('id');
            // echo "<pre>";
            // print_r($this->input->post());
            // echo "</pre>";
            // exit();
            ...
            $respo = $this->interface_m->edit($idinterface,$input);
            if($respo->status){             
                $data['success']=$respo->message;                  
            } else {                
                $data['error']=$respo->message;
        ...
        $detail = $this->interface_m->get_detail($id);       
        if($detail->status){
            $detail = $detail->data;
            $data["interface_code"] = $detail->interface_code;
		    ...
		    $this->load->view('interface_edit_v', $data);
        } else {
	...
	public function delete($id,$other=""){       
		if($id){
            $respo = $this->interface_m->del($id);
            if($respo->status){             
				redirect(base_url().'interface/?alert=success') ; 			
            } else {                
				redirect(base_url().'interface/?alert=failed') ; 			
            }                       
        }        
		redirect(base_url().'interface/?alert=failed') ; 			
	}
